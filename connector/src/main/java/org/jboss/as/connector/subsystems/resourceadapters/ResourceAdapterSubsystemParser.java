...
public final class ResourceAdapterSubsystemParser implements XMLStreamConstants, XMLElementReader<List<ModelNode>>,
    ...
    private void writeConDef(XMLExtendedStreamWriter streamWriter, ModelNode conDef, final String poolName, final boolean isXa) throws XMLStreamException {
        ...
        if (conDef.hasDefined(RECOVERY_USERNAME.getName()) || conDef.hasDefined(RECOVERY_PASSWORD.getName())
                || conDef.hasDefined(RECOVERY_SECURITY_DOMAIN.getName()) || conDef.hasDefined(RECOVERY_PLUGIN_CLASSNAME.getName())
                || conDef.hasDefined(RECOVERY_PLUGIN_PROPERTIES.getName()) || conDef.hasDefined(NO_RECOVERY.getName())
                || conDef.hasDefined(ELYTRON_ENABLED.getName())) {
            ...
            if (conDef.hasDefined(RECOVERY_PLUGIN_CLASSNAME.getName()) || conDef.hasDefined(RECOVERY_PLUGIN_PROPERTIES.getName())) {
                streamWriter.writeStartElement(Recovery.Tag.RECOVER_PLUGIN.getLocalName());
                RECOVERY_PLUGIN_CLASSNAME.marshallAsAttribute(conDef, streamWriter);
                if (conDef.hasDefined(RECOVERY_PLUGIN_PROPERTIES.getName())) {
                    for (Property property : conDef.get(RECOVERY_PLUGIN_PROPERTIES.getName()).asPropertyList()) {
